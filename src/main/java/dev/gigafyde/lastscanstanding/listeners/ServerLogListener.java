...
import dev.gigafyde.lastscanstanding.helpers.Emotes;
...
public class ServerLogListener extends ListenerAdapter {
    public static Logger logger = LoggerFactory.getLogger("ServerLogListener");
    ...
    public void onGuildMessageUpdate(GuildMessageUpdateEvent event) {
        ...
        if (after.getAuthor().isBot()) return; // Ignore bots
        ...
        log(String.format(Emotes.EDIT + " %s | %s %s's message (`%s`) has been edited\nBefore: `%s`\nAfter: `%s`",
                ...
                getUser(before),
                after.getId(),
                before.getContentStripped().replace("`", ""),
                after.getContentStripped().replace("`", "")));
    }
